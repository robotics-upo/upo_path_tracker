...
void SFMNav::odomReceived(const nav_msgs::Odometry::ConstPtr &odom)
{
  ...
  agents[0].linearVelocity = std::sqrt(odom->twist.twist.linear.x * odom->twist.twist.linear.x +
...
void SFMNav::poseReceived(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &pose)
{
  ...
  // agents[0].linearVelocity = std::sqrt(odom->twist.twist.linear.x*odom->twist.twist.linear.x
  // +odom->twist.twist.linear.y*odom->twist.twist.linear.y); agents[0].angularVelocity = odom->twist.twist.angular.z;
...
void SFMNav::publishForceMarker(unsigned index, const std_msgs::ColorRGBA &color, const utils::Vector2d &force,
